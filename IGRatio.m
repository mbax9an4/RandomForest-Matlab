function [gainRatio] = IGRatio(labels,leftLabels,rightLabels)
...
intrinsicInfo = exLeft/examples * log2(exLeft/examples) + exRight/examples * log2(exRight/examples);
intrinsicInfo = intrinsicInfo * -1;

splitIG = double(beforeSplit - average);

gainRatio = splitIG/intrinsicInfo;
if isnan(gainRatio)
    gainRatio = 0;
end
end
