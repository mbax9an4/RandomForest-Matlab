%recursive function that trains a decision tree given a depth, is also used
%by the RF ensemble method, ad given the approriate inputs it trains with
%random feature subspace and different feature selection methods see
%RandomForest file for explanation of input arguments
function treeNode = ID3(tree,data,labels,depth,fChosen,fNo,in)
...
if fNo > 0
    [skipF]=randomizeFeatures(features,fNo);
else
    skipF = [];
end
...
if size(leftSample,2) > 1 && bestFeature ~= 0
    ...
    tree.add(ID3(Tree(-10,intmax,-1,[]),leftSample,leftLabels,depth-1,fChosen,fNo,in));
end

if size(rightSample,2) > 1 && bestFeature ~= 0
    rightSample(:,bestFeature) = [];
    fChosen=sort(fChosen);
    tree.add(ID3(Tree(-10,intmax,-1,[]),rightSample,rightLabels,depth-1,fChosen,fNo,in));
end
